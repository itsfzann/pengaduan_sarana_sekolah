...
async function muatTabel(table, elementId, keyword = '') {
    ...
    try {
        ...
        const tbody = tableEl.querySelector('tbody');
        if(tbody) tbody.innerHTML = '<tr><td colspan="100%">Gagal mengambil data dari server.</td></tr>';
    }
}
...
async function submitData(event, table, elementId) {
    ...
    const idVal = pkInput ? pkInput.value : '';

    let url = `${API}?table=${table}`;
    if (action === 'update' && idVal) {
        url += `&action=update&id=${idVal}`;
    }
    ...
    relasiElements.forEach(async (select) => {
        ...
        try {
            ...
            if (Array.isArray(data)) {
                const defaultOption = select.options[0] ? select.options[0].outerHTML : '<option value="">-- Pilih --</option>';
                select.innerHTML = defaultOption + data.map(row => {
